...
This branch completes the code-layout phase of the refactor. The current research
and review instructions remain available. The small single-skill plugin, remote
catalog retrieval/cache helper and release build are the next phase; this checkout
does not yet provide that plugin.

## Repository layout

```text
skills/     Central index, research instructions and shared Python helpers
plugin/     Reserved for plugin configuration and packaging in phase two
...
[skills/index.md](skills/index.md) lists first-party skills and pinned third-party
instruction links. Upstream source trees are not installed. Dependencies must be
checked before using a linked method; a link does not supply an MCP service,
proprietary terminal or data license. See [third-party notices](THIRD_PARTY_NOTICES.md).
...
point is currently `skills/finrunbook/SKILL.md`; topic discovery starts at
`skills/finrunbook-radar/SKILL.md`. Codex collects and reviews evidence. Python
...
python3 skills/finrun/scripts/new_run.py \
  --subject "Example company" --request "Analyze the latest earnings" \
  --request-language en
...
The current workflow still requires its financial editorial pass and validation
contract. The validator checks structure, provenance and supported arithmetic;
...
Plugin implementation, installation, publication and deployment are separate work.
